...
public class ProductController(IProductService service) : BaseController
{
    ...
    [HttpPost] public async Task<IActionResult> Create([FromBody] ProductCreateInfo entity)
        => (await service.CreateAsync(entity)).ToActionResult();

    [HttpPut("{id:int}")] public async Task<IActionResult> Update([FromRoute] int id, [FromBody] ProductUpdateInfo entity)
